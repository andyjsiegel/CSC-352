...
// Structure for a node (city) in the graph
typedef struct Node {
    char* name;
    int minDist;
    int marked;
    ...
    while (getline(&line, &len, stdin) != -1) {
        ...
        if (sscanf(line, "%64s %64s", name1, name2) == 2) {
            ...
            if (start_node == NULL || end_node == NULL) {
                // As per instructions, ignore if city not found
                continue;
            }
...
// Function to find a node by name, or create it if it doesn't exist
Node* find_or_create_node(Graph* graph, const char* name) {
...
void add_edge(Node* src, Node* dest, int dist) {
    ...
    while (current_edge != NULL) {
        if (current_edge->dest == dest) {
            // Edge already exists, do nothing as per instructions
            return;
        }
